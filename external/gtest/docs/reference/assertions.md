...
When comparing a pointer to `NULL`, use `EXPECT_EQ(`*`ptr_`*`, nullptr)` instead
of `EXPECT_EQ(`*`ptr_`*`, NULL)`.
...
When comparing a pointer to `NULL`, use `EXPECT_NE(`*`ptr_`*`, nullptr)` instead
of `EXPECT_NE(`*`ptr_`*`, NULL)`.
